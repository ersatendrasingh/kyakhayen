...
export function getMealTimeColumns(
  onEdit: (mealTime: MealTimeRecord) => void,
  onPublishedChange: (mealTime: MealTimeRecord, isPublished: boolean) => void,
  onDelete: (mealTime: MealTimeRecord) => void,
  onMoveUp: (mealTime: MealTimeRecord) => void,
  onMoveDown: (mealTime: MealTimeRecord) => void,
  isFirst: (mealTime: MealTimeRecord) => boolean,
  isLast: (mealTime: MealTimeRecord) => boolean,
  reordering: boolean,
  publishingId: string | null
): ColumnDef<MealTimeRecord>[] {
  return [
    {
      id: "select",
      header: ({ table }) => (
        <Checkbox
          aria-label="Select all meal times"
          checked={
    ...
    {
      accessorKey: "title",
      header: ({ column }) => <SortHeader column={column}>Meal Time</SortHeader>,
      cell: ({ row }) => {
        ...
        return (
          ...
          >
            ...
            <span className="min-w-0">
              <span className="block truncate font-semibold">{mealTime.title}</span>
    ...
    {
      id: "recipes",
      accessorFn: (mealTime) => mealTime._count.recipeMealTime,
      header: ({ column }) => <SortHeader column={column}>Recipes</SortHeader>,
      cell: ({ row }) => (
        <Badge variant="secondary">{row.original._count.recipeMealTime} linked</Badge>
      ),
    },
    {
      ...
      cell: ({ row }) => (
        <DropdownMenu>
          <DropdownMenuTrigger asChild>
            <Button variant="ghost" size="icon-sm" aria-label="Open meal time actions">
              <EllipsisVertical />
            </Button>
          </DropdownMenuTrigger>
