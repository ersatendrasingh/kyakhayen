...
import type { MealTimeImportRow } from "@/components/admin/recipe-meal-times/meal-time-types";
import {
  downloadMealTimeTemplate,
  parseMealTimeCsv,
} from "@/components/admin/recipe-meal-times/meal-time-utils";
...
type MealTimeImportDialogProps = {
  open: boolean;
  onOpenChange: (open: boolean) => void;
  onImported: () => void;
};

export function MealTimeImportDialog({
  open,
  onOpenChange,
  onImported,
}: MealTimeImportDialogProps) {
  const [fileName, setFileName] = useState("");
  const [rows, setRows] = useState<MealTimeImportRow[]>([]);
  ...
  const onFileChange = async (file: File | undefined) => {
    ...
    try {
      ...
      const parsed = await parseMealTimeCsv(file);
      setRows(parsed);
      toast.success(`${parsed.length} meal times ready for import`);
    } catch (error) {
      setRows([]);
  ...
  const onImport = async () => {
    ...
    try {
      ...
      toast.success(`${imported} meal times imported successfully`);
      setRows([]);
      setFileName("");
      onOpenChange(false);
      onImported();
      ...
      toast.error(error instanceof Error ? error.message : "Unable to import meal times.");
    } finally {
      setImporting(false);
    }
  };
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-h-[calc(100vh-2rem)] gap-0 overflow-hidden rounded-3xl border-border/70 p-0 sm:max-w-3xl">
        <DialogHeader className="border-b bg-card/75 px-6 py-6">
          <DialogTitle className="text-xl">Import meal times</DialogTitle>
          <DialogDescription>
        ...
        <div className="space-y-5 overflow-y-auto p-6">
          <div className="flex flex-col gap-3 sm:flex-row">
            ...
            <Button
              ...
              onClick={downloadMealTimeTemplate}
            >
              <FileSpreadsheet />
              Download Template
            </Button>
