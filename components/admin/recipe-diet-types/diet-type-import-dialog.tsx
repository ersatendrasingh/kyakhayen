...
import type { DietTypeImportRow } from "@/components/admin/recipe-diet-types/diet-type-types";
import {
  downloadDietTypeTemplate,
  parseDietTypeCsv,
} from "@/components/admin/recipe-diet-types/diet-type-utils";
...
type DietTypeImportDialogProps = {
  open: boolean;
  onOpenChange: (open: boolean) => void;
  onImported: () => void;
};

export function DietTypeImportDialog({ open, onOpenChange, onImported }: DietTypeImportDialogProps) {
  const [fileName, setFileName] = useState("");
  const [rows, setRows] = useState<DietTypeImportRow[]>([]);
  ...
  const onFileChange = async (file: File | undefined) => {
    ...
    try {
      ...
      const parsed = await parseDietTypeCsv(file);
      setRows(parsed);
      toast.success(`${parsed.length} diet types ready for import`);
    } catch (error) {
      setRows([]);
  ...
  const onImport = async () => {
    ...
    try {
      ...
      toast.success(`${imported} diet types imported successfully`);
      setRows([]);
      setFileName("");
      onOpenChange(false);
      onImported();
      ...
      toast.error(error instanceof Error ? error.message : "Unable to import diet types.");
    } finally {
      setImporting(false);
    }
  };
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-h-[calc(100vh-2rem)] gap-0 overflow-hidden rounded-3xl border-border/70 p-0 sm:max-w-3xl">
        <DialogHeader className="border-b bg-card/75 px-6 py-6">
          <DialogTitle className="text-xl">Import diet types</DialogTitle>
        ...
        <div className="space-y-5 overflow-y-auto p-6">
          <div className="flex flex-col gap-3 sm:flex-row">
            ...
            <Button type="button" variant="outline" className="min-h-14 rounded-2xl" onClick={downloadDietTypeTemplate}>
              <FileSpreadsheet />
              Download Template
            </Button>
          </div>
