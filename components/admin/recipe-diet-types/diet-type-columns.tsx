...
export function getDietTypeColumns(
  onEdit: (dietType: DietTypeRecord) => void,
  onPublishedChange: (dietType: DietTypeRecord, isPublished: boolean) => void,
  onDelete: (dietType: DietTypeRecord) => void,
  onMoveUp: (dietType: DietTypeRecord) => void,
  onMoveDown: (dietType: DietTypeRecord) => void,
  isFirst: (dietType: DietTypeRecord) => boolean,
  isLast: (dietType: DietTypeRecord) => boolean,
  reordering: boolean,
  publishingId: string | null
): ColumnDef<DietTypeRecord>[] {
  return [
    {
      id: "select",
      header: ({ table }) => (
    ...
    {
      accessorKey: "title",
      header: ({ column }) => <SortHeader column={column}>Diet Type</SortHeader>,
      cell: ({ row }) => (
        <button
          type="button"
    ...
    {
      id: "recipes",
      accessorFn: (dietType) => dietType._count.recipeDietType,
      header: ({ column }) => <SortHeader column={column}>Recipes</SortHeader>,
      cell: ({ row }) => (
        <Badge variant="secondary">{row.original._count.recipeDietType} linked</Badge>
      ),
    },
    {
      ...
      cell: ({ row }) => (
        <DropdownMenu>
          <DropdownMenuTrigger asChild>
            <Button variant="ghost" size="icon-sm" aria-label="Open diet type actions">
              <EllipsisVertical />
            </Button>
          </DropdownMenuTrigger>
