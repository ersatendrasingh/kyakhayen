...
import type { NutrientImportRow } from "@/components/admin/recipe-nutrients/nutrient-types";
import {
  downloadNutrientTemplate,
  parseNutrientCsv,
} from "@/components/admin/recipe-nutrients/nutrient-utils";
...
type NutrientImportDialogProps = {
  open: boolean;
  onOpenChange: (open: boolean) => void;
  onImported: () => void;
};

export function NutrientImportDialog({
  open,
  onOpenChange,
  onImported,
}: NutrientImportDialogProps) {
  const [fileName, setFileName] = useState("");
  const [rows, setRows] = useState<NutrientImportRow[]>([]);
  ...
  const onFileChange = async (file: File | undefined) => {
    ...
    try {
      ...
      const parsed = await parseNutrientCsv(file);
      setRows(parsed);
      toast.success(`${parsed.length} nutrients ready for import`);
    } catch (error) {
      setRows([]);
  ...
  const onImport = async () => {
    ...
    try {
      ...
      toast.success(`${imported} nutrients imported successfully`);
      setRows([]);
      setFileName("");
      onOpenChange(false);
      onImported();
      ...
      toast.error(error instanceof Error ? error.message : "Unable to import nutrients.");
    } finally {
      setImporting(false);
    }
  };
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-h-[calc(100vh-2rem)] gap-0 overflow-hidden rounded-3xl border-border/70 p-0 sm:max-w-3xl">
        <DialogHeader className="border-b bg-card/75 px-6 py-6">
          <DialogTitle className="text-xl">Import nutrients</DialogTitle>
          <DialogDescription>
        ...
        <div className="space-y-5 overflow-y-auto p-6">
          <div className="flex flex-col gap-3 sm:flex-row">
            ...
            <Button
              ...
              onClick={downloadNutrientTemplate}
            >
              <FileSpreadsheet />
              Download Template
            </Button>
