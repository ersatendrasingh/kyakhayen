...
import type { CuisineImportRow } from "@/components/admin/recipe-cuisines/cuisine-types";
import {
  downloadCuisineTemplate,
  parseCuisineCsv,
} from "@/components/admin/recipe-cuisines/cuisine-utils";
...
type CuisineImportDialogProps = {
  open: boolean;
  onOpenChange: (open: boolean) => void;
  onImported: () => void;
};

export function CuisineImportDialog({
  open,
  onOpenChange,
  onImported,
}: CuisineImportDialogProps) {
  const [fileName, setFileName] = useState("");
  const [rows, setRows] = useState<CuisineImportRow[]>([]);
  ...
  const onFileChange = async (file: File | undefined) => {
    ...
    try {
      ...
      const parsed = await parseCuisineCsv(file);
      setRows(parsed);
      toast.success(`${parsed.length} cuisines ready for import`);
    } catch (error) {
      setRows([]);
  ...
  const onImport = async () => {
    ...
    try {
      ...
      toast.success(`${imported} cuisines imported successfully`);
      setRows([]);
      setFileName("");
      onOpenChange(false);
      onImported();
      ...
      toast.error(error instanceof Error ? error.message : "Unable to import cuisines.");
    } finally {
      setImporting(false);
    }
  };
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-h-[calc(100vh-2rem)] gap-0 overflow-hidden rounded-3xl border-border/70 p-0 sm:max-w-3xl">
        <DialogHeader className="border-b bg-card/75 px-6 py-6">
          <DialogTitle className="text-xl">Import cuisines</DialogTitle>
          <DialogDescription>
        ...
        <div className="space-y-5 overflow-y-auto p-6">
          <div className="flex flex-col gap-3 sm:flex-row">
            ...
            <Button
              ...
              onClick={downloadCuisineTemplate}
            >
              <FileSpreadsheet />
              Download Template
            </Button>
