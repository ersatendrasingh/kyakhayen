...
export function getCuisineColumns(
  onEdit: (cuisine: CuisineRecord) => void,
  onPublishedChange: (cuisine: CuisineRecord, isPublished: boolean) => void,
  onDelete: (cuisine: CuisineRecord) => void,
  onMoveUp: (cuisine: CuisineRecord) => void,
  onMoveDown: (cuisine: CuisineRecord) => void,
  isFirst: (cuisine: CuisineRecord) => boolean,
  isLast: (cuisine: CuisineRecord) => boolean,
  reordering: boolean,
  publishingId: string | null
): ColumnDef<CuisineRecord>[] {
  return [
    {
      id: "select",
      header: ({ table }) => (
        <Checkbox
          aria-label="Select all cuisines"
          checked={
    ...
    {
      accessorKey: "title",
      header: ({ column }) => <SortHeader column={column}>Cuisine</SortHeader>,
      cell: ({ row }) => {
        ...
        return (
          ...
          >
            ...
            <span className="min-w-0">
              <span className="block truncate font-semibold">{cuisine.title}</span>
    ...
    {
      id: "recipes",
      accessorFn: (cuisine) => cuisine._count.recipeCuisine,
      header: ({ column }) => <SortHeader column={column}>Recipes</SortHeader>,
      cell: ({ row }) => (
        <Badge variant="secondary">{row.original._count.recipeCuisine} linked</Badge>
      ),
    },
    {
      ...
      cell: ({ row }) => (
        <DropdownMenu>
          <DropdownMenuTrigger asChild>
            <Button variant="ghost" size="icon-sm" aria-label="Open cuisine actions">
              <EllipsisVertical />
            </Button>
          </DropdownMenuTrigger>
