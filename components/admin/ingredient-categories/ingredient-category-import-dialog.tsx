...
import type { IngredientCategoryImportRow } from "@/components/admin/ingredient-categories/ingredient-category-types";
import {
  downloadIngredientCategoryTemplate,
  parseIngredientCategoryCsv,
} from "@/components/admin/ingredient-categories/ingredient-category-utils";
...
export function IngredientCategoryImportDialog({
  open,
  onOpenChange,
  onImported,
}: {
  ...
  const [rows, setRows] = useState<IngredientCategoryImportRow[]>([]);
  ...
  const onFileChange = async (file: File | undefined) => {
    ...
    try {
      ...
      const parsed = await parseIngredientCategoryCsv(file);
      setRows(parsed);
      toast.success(`${parsed.length} ingredient categories ready for import`);
    } catch (error) {
      setRows([]);
  ...
  const onImport = async () => {
    ...
    try {
      ...
      toast.success(`${imported} ingredient categories imported successfully`);
      setRows([]);
      setFileName("");
      onOpenChange(false);
      onImported();
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-h-[calc(100vh-2rem)] gap-0 overflow-hidden rounded-3xl border-border/70 p-0 sm:max-w-3xl">
        <DialogHeader className="border-b bg-card/75 px-6 py-6">
          <DialogTitle className="text-xl">Import ingredient categories</DialogTitle>
          <DialogDescription>
            Upload a CSV file with `name` and optional `imageUrl` columns.
          </DialogDescription>
        </DialogHeader>
        <div className="space-y-5 overflow-y-auto p-6">
          <div className="flex flex-col gap-3 sm:flex-row">
            ...
            <Button type="button" variant="outline" className="min-h-14 rounded-2xl" onClick={downloadIngredientCategoryTemplate}>
              <FileSpreadsheet />
              Download Template
            </Button>
          </div>
            ...
            <div className="max-h-72 divide-y overflow-y-auto">
              {previewRows.length ? (
                previewRows.map((row, index) => (
                  <div key={`${row.name}-${index}`} className="grid grid-cols-[1fr_1.35fr] gap-4 px-4 py-3 text-sm">
