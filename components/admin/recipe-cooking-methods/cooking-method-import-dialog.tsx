...
import type { CookingMethodImportRow } from "@/components/admin/recipe-cooking-methods/cooking-method-types";
import {
  downloadCookingMethodTemplate,
  parseCookingMethodCsv,
} from "@/components/admin/recipe-cooking-methods/cooking-method-utils";
...
type CookingMethodImportDialogProps = {
  open: boolean;
  onOpenChange: (open: boolean) => void;
  onImported: () => void;
};

export function CookingMethodImportDialog({
  open,
  onOpenChange,
  onImported,
}: CookingMethodImportDialogProps) {
  const [fileName, setFileName] = useState("");
  const [rows, setRows] = useState<CookingMethodImportRow[]>([]);
  ...
  const onFileChange = async (file: File | undefined) => {
    ...
    try {
      ...
      const parsed = await parseCookingMethodCsv(file);
      setRows(parsed);
      toast.success(`${parsed.length} cooking methods ready for import`);
    } catch (error) {
      setRows([]);
  ...
  const onImport = async () => {
    ...
    try {
      ...
      toast.success(`${imported} cooking methods imported successfully`);
      setRows([]);
      setFileName("");
      onOpenChange(false);
      onImported();
      ...
      toast.error(error instanceof Error ? error.message : "Unable to import cooking methods.");
    } finally {
      setImporting(false);
    }
  };
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-h-[calc(100vh-2rem)] gap-0 overflow-hidden rounded-3xl border-border/70 p-0 sm:max-w-3xl">
        <DialogHeader className="border-b bg-card/75 px-6 py-6">
          <DialogTitle className="text-xl">Import cooking methods</DialogTitle>
          <DialogDescription>
        ...
        <div className="space-y-5 overflow-y-auto p-6">
          <div className="flex flex-col gap-3 sm:flex-row">
            ...
            <Button
              ...
              onClick={downloadCookingMethodTemplate}
            >
              <FileSpreadsheet />
              Download Template
            </Button>
