...
function SortHeader({
  ...
  column: Column<CookingMethodRecord>;
  children: ReactNode;
}) {
  return (
    <Button
...
export function getCookingMethodColumns(
  onEdit: (method: CookingMethodRecord) => void,
  onPublishedChange: (method: CookingMethodRecord, isPublished: boolean) => void,
  onDelete: (method: CookingMethodRecord) => void,
  onMoveUp: (method: CookingMethodRecord) => void,
  onMoveDown: (method: CookingMethodRecord) => void,
  isFirst: (method: CookingMethodRecord) => boolean,
  isLast: (method: CookingMethodRecord) => boolean,
  reordering: boolean,
  publishingId: string | null
): ColumnDef<CookingMethodRecord>[] {
  return [
    {
      id: "select",
      header: ({ table }) => (
        <Checkbox
          aria-label="Select all cooking methods"
          checked={
    ...
    {
      accessorKey: "title",
      header: ({ column }) => <SortHeader column={column}>Cooking Method</SortHeader>,
      cell: ({ row }) => {
    ...
    {
      id: "recipes",
      accessorFn: (method) => method._count.recipeCookingMethod,
      header: ({ column }) => <SortHeader column={column}>Recipes</SortHeader>,
      cell: ({ row }) => (
        <Badge variant="secondary">
          {row.original._count.recipeCookingMethod} linked
        </Badge>
      ),
    },
    {
      ...
      cell: ({ row }) => (
        <DropdownMenu>
          <DropdownMenuTrigger asChild>
            <Button variant="ghost" size="icon-sm" aria-label="Open cooking method actions">
              <EllipsisVertical />
            </Button>
          </DropdownMenuTrigger>
