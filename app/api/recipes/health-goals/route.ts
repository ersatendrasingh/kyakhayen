...
export async function POST(req: Request) {
  try {
    ...
    const healthGoal = await db.healthGoals.create({
      data: {
        title,
        slug,
      },
    ...
    if (file instanceof Blob) {
      ...
      const fileName = `healthGoals/${healthGoal.id}/${file.name}`;
    ...
    console.log("[HEALTH_GOALS]", error);
