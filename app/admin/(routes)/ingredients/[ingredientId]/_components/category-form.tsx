...
const formSchema = z.object({
  ingredientCategoriesId: z.string().min(1),
});
...
export const CategoryForm = ({
  ...
  const form = useForm<z.infer<typeof formSchema>>({
    ...
    defaultValues: {
      ingredientCategoriesId: initialData?.ingredientCategoriesId || "",
    },
  });
  ...
  const onSubmit = async (values: z.infer<typeof formSchema>) => {
    try {
      ...
      toast.error("Something went wrong while updating ingredient category", {
        duration: 5000,
      });
    }
  };
  ...
  const selectedOption = options.find(
    (option) => option.value === initialData.ingredientCategoriesId
  );
  return (
    <div className="mt-6 border rounded-md p-4 bg-slate-100">
      <div className="flex items-center justify-between font-medium">
        ...
        <Button onClick={toggleEdit} variant="ghost">
          {isEditing ? (
            ...
            <>
              {initialData.ingredientCategoriesId ? (
                <>
      ...
      {!isEditing && (
        <p
          className={cn(
            "text-sm mt-2",
            !initialData.ingredientCategoriesId && "italic text-slate-500"
          )}
        >
