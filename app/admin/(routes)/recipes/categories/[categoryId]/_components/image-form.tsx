...
import { RecipeCategories } from "@prisma/client";
...
export const ImageForm = ({ initialData, categoryId }: ImageFormProps) => {
  ...
  const onSubmit = async (fileUrl: string) => {
    ...
    try {
      await axios.patch(`/api/recipes/categories/${categoryId}`, imageUrl);
      ...
      toast.error("Something went wrong while updating category image", {
        position: "top-center",
        autoClose: 5000,
      });
    }
