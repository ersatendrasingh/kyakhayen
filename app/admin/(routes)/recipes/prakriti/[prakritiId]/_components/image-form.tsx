...
import { Prakriti } from "@prisma/client";

interface ImageFormProps {
  initialData: Prakriti;
  prakritiId: string;
}

export const ImageForm = ({ initialData, prakritiId }: ImageFormProps) => {
  ...
  const onSubmit = async (fileUrl: string) => {
    ...
    try {
      await axios.patch(`/api/recipes/prakriti/${prakritiId}`, imageUrl);
      toast.success("Prakriti image updated successfully", {
        position: "top-center",
        autoClose: 5000,
      });
      toggleEdit();
      ...
      toast.error("Something went wrong while updating prakriti image", {
        position: "top-center",
        autoClose: 5000,
      });
    }
