...
import { HealthGoals } from "@prisma/client";

interface ImageFormProps {
  initialData: HealthGoals;
  healthGoalId: string;
}

export const ImageForm = ({ initialData, healthGoalId }: ImageFormProps) => {
  ...
  const onSubmit = async (fileUrl: string) => {
    ...
    try {
      await axios.patch(`/api/recipes/health-goals/${healthGoalId}`, imageUrl);
      toast.success("Health goal image updated successfully", {
        position: "top-center",
        autoClose: 5000,
      });
      toggleEdit();
      ...
      toast.error("Something went wrong while updating health goal image", {
        position: "top-center",
        autoClose: 5000,
      });
    }
