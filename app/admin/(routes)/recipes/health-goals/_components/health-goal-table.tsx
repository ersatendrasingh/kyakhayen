...
interface HealthGoalTableProps {
  initialHealthGoals: HealthGoal[];
}

const HealthGoalTable = ({ initialHealthGoals }: HealthGoalTableProps) => {
  ...
  const onReorder = async (updateData: { id: string; position: number }[]) => {
    try {
      ...
      await axios.put(`/api/recipes/health-goals/reorder`, {
        list: updateData,
      });
      toast.success("Health goals reordered successfully", {
        position: "top-center",
        autoClose: 5000,
      });
    } catch {
      toast.error("Something went wrong while reordering health goals", {
        position: "top-center",
        autoClose: 5000,
      });
    } finally {
  ...
  const getCurrentPageItems = () => {
    ...
    return healthGoals.slice(startIndex, endIndex);
  };
  ...
  const columns: ColumnDef<HealthGoal>[] = [
    {
        ...
        return (
          <DropdownMenu>
            ...
            <DropdownMenuContent align="end">
              <Link href={`/admin/recipes/health-goals/${id}`}>
    ...
    <>
      <div className="relative">
        <DataTable
          ...
          filterPlaceholder="Health Goals"
        />

        <Pagination
          currentPage={currentPage}
          totalItems={healthGoals.length}
...
export default HealthGoalTable;
