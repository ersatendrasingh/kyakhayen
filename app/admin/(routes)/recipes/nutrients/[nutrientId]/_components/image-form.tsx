...
export const ImageForm = ({ initialData, nutrientId }: ImageFormProps) => {
  ...
  const onSubmit = async (fileUrl: string) => {
    ...
    try {
      await axios.patch(`/api/recipes/nutrients/${nutrientId}`, imageUrl);
      toast.success("Nutrient image updated successfully", {
        position: "top-center",
        autoClose: 5000,
      });
      toggleEdit();
      ...
      toast.error("Something went wrong while updating nutrient image", {
        position: "top-center",
        autoClose: 5000,
      });
    }
