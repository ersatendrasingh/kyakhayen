...
export const ImageForm = ({ initialData, allergyId }: ImageFormProps) => {
  ...
  const onSubmit = async (fileUrl: string) => {
    ...
    try {
      await axios.patch(`/api/recipes/allergies/${allergyId}`, imageUrl);
      toast.success("Allergy image updated successfully", {
        position: "top-center",
        autoClose: 5000,
      });
      toggleEdit();
      ...
      toast.error("Something went wrong while updating allergy image", {
        position: "top-center",
        autoClose: 5000,
      });
    }
