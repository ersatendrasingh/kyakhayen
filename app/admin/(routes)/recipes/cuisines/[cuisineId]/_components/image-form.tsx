...
export const ImageForm = ({ initialData, cuisineId }: ImageFormProps) => {
  ...
  const onSubmit = async (fileUrl: string) => {
    ...
    try {
      await axios.patch(`/api/recipes/cuisines/${cuisineId}`, imageUrl);
      toast.success("Cuisine image updated successfully", {
        position: "top-center",
        autoClose: 5000,
      });
      toggleEdit();
      ...
      toast.error("Something went wrong while updating cuisine image", {
        position: "top-center",
        autoClose: 5000,
      });
    }
