...
interface CuisinesTableProps {
  initialCuisines: Cuisine[];
}

const CuisinesTable = ({ initialCuisines }: CuisinesTableProps) => {
  ...
  const onReorder = async (updateData: { id: string; position: number }[]) => {
    try {
      ...
      await axios.put(`/api/recipes/cuisines/reorder`, {
        list: updateData,
      });
      ...
      toast.error("Something went wrong while reordering cuisines", {
        position: "top-center",
        autoClose: 5000,
      });
    } finally {
  ...
  const getCurrentPageItems = () => {
    ...
    return cuisines.slice(startIndex, endIndex);
  };
