...
interface ImageFormProps {
  initialData: RecipeTypes;
  recipeTypeId: string;
}

export const ImageForm = ({ initialData, recipeTypeId }: ImageFormProps) => {
  ...
  const onSubmit = async (fileUrl: string) => {
    ...
    try {
      await axios.patch(`/api/recipes/recipe-types/${recipeTypeId}`, imageUrl);
      toast.success("Recipe type image updated successfully", {
        position: "top-center",
        autoClose: 5000,
      });
      toggleEdit();
      ...
      toast.error("Something went wrong while updating recipe type image", {
        position: "top-center",
        autoClose: 5000,
      });
    }
