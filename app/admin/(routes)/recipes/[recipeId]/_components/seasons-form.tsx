...
export const SeasonsForm = ({
  ...
  const onSubmit = async (values: z.infer<typeof formSchema>) => {
    try {
      ...
      toast.success("Recipe season updated successfully", {
        duration: 5000,
      });
      toggleEdit();
      router.refresh();
    } catch {
      toast.error("Something went wrong while updating recipe season", {
        duration: 5000,
      });
    }
  };
          ...
          {isEditing ? (
            ...
            <>
              {initialData.recipeDifficultyId ? (
                <>
                  <Pencil className="w-6 h-6 pr-2" /> Edit season
                </>
              ) : (
                <>
