...
const formSchema = z.object({
  metaTitle: z.string().min(1, { message: "Recipe Meta Title is required" }),
  metaDescription: z
    .string()
    .min(1, { message: "Recipe Meta Description is required" }),
  metaSlug: z.string().min(1, { message: "Recipe Meta Slug is required" }),
});

export const RecipeMetaDataForm = ({
  initialData,
  recipeId,
  ...
  const onSubmit = async (values: z.infer<typeof formSchema>) => {
    try {
      ...
      toast.success("Recipe meta data updated successfully", {
        duration: 5000,
      });
      toggleEdit();
      router.refresh();
    } catch {
      toast.error("Something went wrong while updating recipe meta data", {
        duration: 5000,
      });
    }
  };
