...
interface ImageFormProps {
  initialData: MealTimes;
  mealTimeId: string;
}

export const ImageForm = ({ initialData, mealTimeId }: ImageFormProps) => {
  ...
  const onSubmit = async (fileUrl: string) => {
    ...
    try {
      await axios.patch(`/api/recipes/meal-time/${mealTimeId}`, imageUrl);
      toast.success("Meal time image updated successfully", {
        position: "top-center",
        autoClose: 5000,
      });
      toggleEdit();
      ...
      toast.error("Something went wrong while updating meal time image", {
        position: "top-center",
        autoClose: 5000,
      });
    }
