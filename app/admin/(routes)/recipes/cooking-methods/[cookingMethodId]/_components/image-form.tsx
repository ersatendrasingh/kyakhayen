...
export const ImageForm = ({ initialData, cookingMethodId }: ImageFormProps) => {
  ...
  const onSubmit = async (fileUrl: string) => {
    ...
    try {
      ...
      toast.success("Cooking method image updated successfully", {
        position: "top-center",
        autoClose: 5000,
      });
      toggleEdit();
      ...
      toast.error("Something went wrong while updating cooking method image", {
        position: "top-center",
        autoClose: 5000,
      });
    }
      ...
      {isEditing && (
        <>
          <FileUpload
            ...
            cookingMethodId={initialData.id}
            onChange={(url) => {
              if (url) onSubmit(url);
            }}
          />
