...
export const ImageForm = ({ initialData, diseaseId }: ImageFormProps) => {
  ...
  const onSubmit = async (fileUrl: string) => {
    ...
    try {
      await axios.patch(`/api/recipes/diseases/${diseaseId}`, imageUrl);
      toast.success("Disease image updated successfully", {
        position: "top-center",
        autoClose: 5000,
      });
      toggleEdit();
      ...
      toast.error("Something went wrong while updating disease image", {
        position: "top-center",
        autoClose: 5000,
      });
    }
